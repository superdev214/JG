...
const MarketplacePage = () => {
  ...
  return (
    <>
      ...
      > */}
        <div tw="min-h-[100%] border-r border-[#393544] bg-[#070215]">
          <div tw="px-6 pt-12 pb-10">
            <Link to="/" tw="flex items-center">
              <img alt="go back" src={iconGoBack} width="6" />
              <span tw="pl-3 text-sm uppercase tracking-[0.22em]">Go back</span>
            </Link>
            <h2 tw="pt-4 text-2xl uppercase">Filter</h2>
          </div>
          <div tw="px-6 py-8 border-t border-b border-[#393544]">
            <h3 tw="text-[16px] uppercase text-white">Price</h3>
            <MultiRangeSlider
              highValue={maxPrice1}
              lowValue={minPrice1}
              max={1000}
            ...
            <div tw="mt-4 relative grid grid-cols-2 gap-x-[26px]">
              <div tw="relative h-[52px] flex items-center border border-[rgba(255, 255, 255, 0.3)]">
                <input
                  max={1000}
                  min={0}
                  tw="appearance-none pl-4 pr-7 w-full outline-none text-base text-white bg-transparent"
                  type="number"
                  value={minPrice}
                  onChange={(e) => setMinPrice1(parseFloat(e.target.value))}
                />
                <span tw="absolute right-4 text-2xl top-[50%] translate-y-[-50%] text-[#6e7588]">
                  $
                </span>
              </div>
              <div tw="relative h-[52px] flex items-center border border-[rgba(255, 255, 255, 0.3)]">
          ...
          {prefix === 'Axie Infinity' ? (
            <>
              <div tw="px-6 pt-8">
                <h3 tw="text-base leading-[130%] uppercase text-[#fff]">
                  Category
                </h3>
                <div tw="pt-1">
                  {tags.types.map((item) => (
        ...
        <div tw="px-2 md:px-5 py-8 min-h-[100%]">
          <p tw="pb-12 font-lumadFree text-[24px]">
            {itemCount}{' '}
            {games.length > 1
          ...
          <div tw="pt-[46px] flex justify-center items-center text-[16px] text-[#857E9A] gap-4">
            <button
              tw="px-4"
              onClick={() => {
                if (pageNumber > 1) {
            ...
            >
              <img alt="prev" src={iconPrev} width={32} />
            </button>
            Page
            <input
              tw="w-[60px] h-[60px] text-[24px] text-center border border-[#fff4] focus:border-[#A237F4] outline-none bg-transparent"
              value={pageNumber1}
            ...
            >
              <img alt="next" src={iconNext} width={32} />
            </button>
          </div>
        </div>
        {/* </SplitterLayout> */}
